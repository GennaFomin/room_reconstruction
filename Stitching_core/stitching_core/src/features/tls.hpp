 ...
 */
//    class CV_EXPORTS TLSDataContainerCustom
//    {
//        protected:
//        TLSDataContainerCustom() { };
//        virtual ~TLSDataContainerCustom();
//
//        /// @deprecated use detachData() instead
//        void  gatherData(std::vector<void*> &data) const;
//        /// get TLS data and detach all data from threads (similar to
//        cleanup() call) void  detachData(std::vector<void*>& data);
//
//        void* getData() const;
//        void  release();
//
...
//
//        public:
//        void cleanup(); //!< Release created TLS data container objects. It is
//        similar to release() call, but it keeps TLS container valid.
//
//        private:
//        // Disable copy/assign (noncopyable pattern)
//        TLSDataContainerCustom(TLSDataContainerCustom &) = delete;
//        TLSDataContainerCustom& operator =(const TLSDataContainerCustom &) =
//        delete;
//    };
 ...
 */
//    template <typename T>
//    class TLSDataCustom : protected TLSDataContainer
//    {
//    public:
//        inline TLSDataCustom() {}
//        inline ~TLSDataCustom() { release(); }
//
//        inline T* get() const   { return (T*)getData(); }  //!< Get data
//        associated with key inline T& getRef() const { T* ptr = (T*)getData();
//        CV_DbgAssert(ptr); return *ptr; }  //!< Get data associated with key
//
...
class TLSDataAccumulator : public TLSData<T> {
   ...
   public:
    ...
    virtual ~TLSDataAccumulator() { release(); }
    ...
    std::vector<T*>& gatherData() {
        ...
        {
            AutoLock lock(mutex);
            detachedData.reserve(dataVoid.size() +
                                 dataFromTerminatedThreads.size());
            for (typename std::vector<T*>::const_iterator i =
                     dataFromTerminatedThreads.begin();
                 i != dataFromTerminatedThreads.end(); ++i) {
            ...
            for (typename std::vector<void*>::const_iterator i =
                     dataVoid.begin();
                 i != dataVoid.end(); ++i) {
