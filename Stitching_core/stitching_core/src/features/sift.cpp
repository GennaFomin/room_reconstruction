...
void SIFT_Impl::buildGaussianPyramid(const Mat& base, std::vector<Mat>& pyr,
                                     int nOctaves) const {
...
void SIFT_Impl::buildDoGPyramid(const std::vector<Mat>& gpyr,
                                std::vector<Mat>& dogpyr) const {
...
static float calcOrientationHist(const Mat& img, Point pt, int radius,
    ...
    float *X = buf.data(), *Y = X + len, *Mag = X, *Ori = Y + len,
          *W = Ori + len;
    ...
    if (USE_AVX2) {
        ...
        for (; k <= len - 8; k += 8) {
            __m256i __bin = _mm256_cvtps_epi32(
                _mm256_mul_ps(__nd360, _mm256_loadu_ps(&Ori[k])));

            __bin = _mm256_sub_epi32(
                __bin,
                _mm256_andnot_si256(_mm256_cmpgt_epi32(__n, __bin), __n));
...
static bool adjustLocalExtrema(const std::vector<Mat>& dog_pyr, KeyPoint& kpt,
    ...
    for (; i < SIFT_MAX_INTERP_STEPS; i++) {
        ...
        Vec3f dD(
            (img.at<sift_wt>(r, c + 1) - img.at<sift_wt>(r, c - 1)) *
                deriv_scale,
            (img.at<sift_wt>(r + 1, c) - img.at<sift_wt>(r - 1, c)) *
                deriv_scale,
            (next.at<sift_wt>(r, c) - prev.at<sift_wt>(r, c)) * deriv_scale);

        float v2 = (float)img.at<sift_wt>(r, c) * 2;
        float dxx =
            (img.at<sift_wt>(r, c + 1) + img.at<sift_wt>(r, c - 1) - v2) *
            second_deriv_scale;
        float dyy =
            (img.at<sift_wt>(r + 1, c) + img.at<sift_wt>(r - 1, c) - v2) *
            second_deriv_scale;
    ...
    kpt.size =
        sigma * powf(2.f, (layer + xi) / nOctaveLayers) * (1 << octv) * 2;
        ...
        for (int r = begin; r < end; r++) {
            ...
            for (int c = SIFT_IMG_BORDER; c < cols - SIFT_IMG_BORDER; c++) {
                ...
                if (std::abs(val) > threshold &&
                    ((val > 0 && val >= currptr[c - 1] &&
                      val >= currptr[c + 1] && val >= currptr[c - step - 1] &&
                      val >= currptr[c - step] &&
                      val >= currptr[c - step + 1] &&
                      val >= currptr[c + step - 1] &&
                      val >= currptr[c + step] &&
                      val >= currptr[c + step + 1] && val >= nextptr[c] &&
                      val >= nextptr[c - 1] && val >= nextptr[c + 1] &&
                      val >= nextptr[c - step - 1] &&
                      val >= nextptr[c - step] &&
                      val >= nextptr[c - step + 1] &&
                      val >= nextptr[c + step - 1] &&
                      val >= nextptr[c + step] &&
...
void SIFT_Impl::findScaleSpaceExtrema(const std::vector<Mat>& gauss_pyr,
    ...
    const int threshold = cvFloor(0.5 * contrastThreshold / nOctaveLayers *
    ...
    for (size_t i = 0; i < kpt_vecs.size(); ++i) {
        keypoints.insert(keypoints.end(), kpt_vecs[i]->begin(),
                         kpt_vecs[i]->end());
    }
}
...
static void calcSIFTDescriptor(const Mat& img, Point2f ptf, float ori,
    ...
    // Clip the radius to the diagonal of the image to avoid autobuffer too
    // large exception
    ...
    float *X = buf.data(), *Y = X + len, *Mag = Y, *Ori = Mag + len,
          *W = Ori + len;
    ...
    for (i = -radius, k = 0; i <= radius; i++)
        for (j = -radius; j <= radius; j++) {
            // Calculate sample's histogram array coords rotated relative to
            // ori. Subtract 0.5 so samples that fall e.g. in the center of row
            // 1 (i.e. r_rot = 1.5) have full weight placed in row 1 after
            // interpolation.
    ...
    if (USE_AVX2) {
        ...
        for (; k <= len - 8; k += 8) {
            ...
            __o0i = _mm256_sub_epi32(
                __o0i,
                _mm256_andnot_si256(_mm256_cmpgt_epi32(__n, __o0i), __n));
...
#undef HIST_SUM_HELPER
    ...
    if (USE_AVX2) {
        ...
        for (k = 0; k <= len - 8; k += 8) {
            __dst = _mm256_loadu_ps(&dst[k]);
            __dst = _mm256_min_ps(
                _mm256_max_ps(_mm256_round_ps(_mm256_mul_ps(__dst, __nrm2),
    ...
    if (!useProvidedKeypoints) {
        ...
        if (firstOctave < 0)
            for (size_t i = 0; i < keypoints.size(); i++) {
                ...
                kpt.octave =
                    (kpt.octave & ~255) | ((kpt.octave + firstOctave) & 255);
                kpt.pt *= scale;
                kpt.size *= scale;
            }
