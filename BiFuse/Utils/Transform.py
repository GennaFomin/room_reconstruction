...
def RodriguesTensor_unitest():
    ...
    batch_tensor = Variable(torch.FloatTensor(batch), requires_grad = True).to("mps")
...
class Depth2Points(nn.Module):
    ...
    def forward(self, x):
        ...
        bs = bs // 6
        grid = self.xyz_grid
        grid = grid.to("mps") if self.CUDA else grid
        all_pts = [] 
        for i in range(bs):
...
class EquirecDepth2Points(nn.Module):
    ...
    def forward(self, depth):
        ...
        pts = []
        grid = self.grid.to("mps") if self.CUDA else self.grid
...
if __name__ == '__main__':
    ...
    batch = Variable(torch.FloatTensor(np.ones([bs * 6, 1, out_dim, out_dim])), requires_grad=False).to("mps")
    ...
    depth = torch.FloatTensor(np.random.rand(bs, 1, h, w) * 0.5).to("mps")
