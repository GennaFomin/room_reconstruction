...
class Cube2Equirec(object):
    ...
    def _ToEquirec(self, batch, mode):
        ...
        if self.CUDA:
            output = Variable(torch.zeros(1, ch, self.output_h, self.output_w), requires_grad=False).to("mps")
        else:
        ...
        for ori in range(6):
            ...
            if self.CUDA:
                masked_grid = Variable(grid * mask.float().expand(-1, -1, -1, 2)).to("mps") # 1, self.output_h, self.output_w, 2
            else:
            ...
            if self.CUDA:
                sampled_image_masked = sampled_image * \
                                    Variable(mask.float().view(1, 1, self.output_h, self.output_w).expand(1, ch, -1, -1)).to("mps")
            else:
                sampled_image_masked = sampled_image * \
...
if __name__ == "__main__":
    ...
    im_tensor = Variable(torch.FloatTensor(im).unsqueeze(0).transpose(1, 3).transpose(2, 3).to("mps"))
