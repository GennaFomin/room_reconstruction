...
class Unpool(nn.Module):
    ...
    def forward(self, x):
        return F.conv_transpose2d(x, self.weights.to("mps"), stride=self.stride, groups=self.num_channels)
...
def e2c(equirectangular):
    cube = Utils.Equirec2Cube.ToCubeTensor(equirectangular.to("mps"))
    return cube

def c2e(cube):
    equirectangular = Utils.Cube2Equirec.ToEquirecTensor(cube.to("mps"))
